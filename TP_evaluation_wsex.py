...
colonnes=[i for i in range(10) if i!=8]
percentage_train=int(0.8*len(X))
X_train1,y_train=X[:percentage_train],y[:percentage_train].ravel()
X_test1,y_test=X[percentage_train:],y[percentage_train:].ravel()
X_train=X_train1[:,colonnes]
X_test=X_test1[:,colonnes]
